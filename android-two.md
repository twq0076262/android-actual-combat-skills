...
[`android`](http://www.csdn.net/tag/android) [`image`](http://www.csdn.net/tag/image) [`layout`](http://www.csdn.net/tag/layout) [`encoding`](http://www.csdn.net/tag/encoding) [`button`](http://www.csdn.net/tag/button)
...
这里唯一值得一说的是文本的颜色。大家看见他是 8 位的，前两位是表示不透明度的，后六位是表示颜色的，三色，范围都是 00～ff。
...
关于不透明度，一般美工会定义。有些要求不透明如 30% 这样的，可以用整型换算一下。00～ff 对应十进制为 0～255，那么 30% 就是255x0.3=76.5，用科学计算机换算为 4c。
